...
class PortfolioController extends Controller
{
         ...
         return [
             'access' => [
                 ...
                 'rules' => [
                     ...
                     [
                         'actions' => ['index', 'view', 'create', 'update', 'delete', 'signup', 'deleteimag', 'deleteteam', 'image', 'show', 'city'],
                         'allow' => true,
                         'roles' => ['admin'],
                     ],
                     [
    ...
    {   
        ...
        if ($model->load(Yii::$app->request->post()) && $coordinate->load(Yii::$app->request->post())) {
            ...
            $model->image1 = $imageName . '.' . $uploadimg->file12->extension;
            
            $source = $uploadimg->getPath().$model->image1;       // Исходный файл
            $dest = $uploadimg->getPaths().$model->image1;   // Файл с результатом работы
            $uploadimg->getResaiz($source, $dest);
    ...
    public function actionUpdate($id)
    {
        ...
        /* ====== File download this model (загрузка фалов для этой модели) ======= */
         if ($model->load(Yii::$app->request->post()) && $coordinate->load(Yii::$app->request->post())) {
            ...
            $model->image1 = $imageName . '.' . $uploadimg->file123->extension;
            ...
            if($model->teams != null){
                $team_find = Portfolio::find()->where(['id' => $model->id])->one();
                //var_dump($team_find->team);die();
                $test = implode(",", $model->teams);
            $model->teams = $test;
            $model->coordinate_id = $coordinate->id;
            $model->team = $model->teams.','.$team_find->team;
            } 
...
return json_encode($output);
    ...
    {   
      ...
      $model = $this->findModel($id);
        ...
        $team_find = Portfolio::find()->where(['id' => $id])->one();
                $test = explode(",", $team_find->team);
                $keys = array_search($key, $test);
                if ($key !== false)
                {
                    unset($test[$keys]);
                }
    ...
    {   
        //$model->removeImages();
        $image = ImagePortfolio::findOne($id);
        //var_dump($image);die();
        //unlink($image->image);
        $image->delete();
